...
<script type="text/javascript" >
	...
	function userSelect() {
		...
		$('body').on('click','#btnSelect',function(){
			...
			$.ajax({
				url:'users/'+userId,
				type:'GET', /* {userid:userId}, 'userid=ssss'*/
				contentType:'application/json;charset=utf-8',
				dataType:'json',
				error:function(xhr,status,msg){
...
/* 			var id = $('input:text[name="id"]').val();
			...
			$.ajax({ 
			    url: "users", 
			    type: 'PUT', //PUT은 JSON구조로만 가능
			  //data: JSON.stringify({id: id, name:name,password: password, role: role }) 밑과 같음
			    data : JSON.stringify($("#form1").serializeObject()),
			    contentType: 'application/json',	//보낼데이터 request 데이터가 json
...
/* 			var id = $('input:text[name="id"]').val();
	...
	function userListResult(data) {
		...
		$.each(data,function(idx,item){
			...
			.appendTo('tbody');	//자식이 부모한테 붙음 appenTo
		});//each
	}//userListResult
</script>
</head>
